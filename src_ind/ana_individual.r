...
if(Sys.info()["sysname"]=="Darwin"){
  ...
  p.file = list.files("../data/", full.names = T, recursive = T)
}
...
r.res0 = as.data.frame(matrix(nrow = nrow(rec.transposon)*nrow(f.sce), ncol = 2))
r.res = cbind(f.sce, r.res0); r.res[,(-1:0)+ncol(r.res)] = NULL
r.res$generation = rep((1:nrow(rec.transposon))-1, each = nrow(f.sce))
r.res$hypothesis = paste0(r.res$jump,r.res$recom)
r.res0 = as.data.frame(matrix(nrow = nrow(rec.transposon)*nrow(f.sce), ncol = max(f.rda$rep)))
